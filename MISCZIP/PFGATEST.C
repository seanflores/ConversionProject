...
static char    DWELL[16];
static char    FREQD[96];
static char    OLDFREQD[96];
static char    INTRPT[16];
static char    MODE[16];
static int     FREQ[18][96];
static int     NEWFREQ(96);
static char    HEAD[18];
static char    FP[96];
static char    OUTPU[18];
static int     SCANBIT;
static char    C[13];
...
C_IMPORT int   __stdcall INDEX(int,int);
int     TICKS (void);
int     INDEX (int, int);
...
for(Y=5; Y<=20; Y+=1)
  {
    for(COLUMN=COLMIN; COLUMN<=6; COLUMN+=1)
      {
        X=6*COLUMN-5;
        if(str_cmp(FREQD(FNINDEX(Y,COLUMN)),"????")!=0)
          {
            color (0,7            );
            OLDLISTSIZE+=1;
          }
        ...
        if(Screen(Y,X,1)fmod16==0)
          {
            ...
            *ASK=0;
            ...
            strcpy(FREQD(FNINDEX(Y,COLUMN)),ASK);
          }
        else
          {
            strcpy(FREQD(FNINDEX(Y,COLUMN)),"????");
          }
      }
  }
longjmp (GosubStack [--GosubNdx],1 );
    ...
    for(COLUMN=COLMIN; COLUMN<=9; COLUMN+=1)
      {
        ...
        if(COLUMN<7)
          {
            strcpy(W,FREQD(FNINDEX(Y,COLUMN)));
            if(str_cmp(W,"????")==0)
              {
                strcpy(W,"    ");
                if(Y<PASTEND(COLUMN))
